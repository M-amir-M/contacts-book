...
class ConfirmValidator extends FieldValidator<bool> {
  ConfirmValidator({required String errorText}) : super(errorText);
...
class IrPhoneValidator extends TextFieldValidator {
  /// regex pattern to validate email inputs.
  final Pattern _emailPattern =
      r"^09[0|1|2|3][0-9]{8}$";

  IrPhoneValidator({required String errorText}) : super(errorText);

  @override
  bool isValid(String? value) => hasMatch(_emailPattern.toString(), value!, caseSensitive: false);
}

class LinkValidator extends TextFieldValidator {
  /// regex pattern to validate email inputs.
  final Pattern _emailPattern =
      r"(https?|http)://([-A-Z0-9.]+)(/[-A-Z0-9+&@#/%=~_|!:,.;]*)?(\?[A-Z0-9+&@#/%=~_|!:‌​,.;]*)?";

  LinkValidator({required String errorText}) : super(errorText);
